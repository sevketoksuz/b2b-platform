...
class IncreaseStockCommandHandlerTest {
    ...
    void setUp() {
        ...
        handler = new IncreaseStockCommandHandler(
    ...
    @Test
    void shouldIncreaseStockAndCreateMovementWhenProductIsActive() {
        ...
        assertEquals(new BigDecimal("15"), result.quantity());
    ...
    @Test
    void shouldThrowExceptionWhenProductBelongsToAnotherCompany() {
        UUID commandCompanyId = UUID.randomUUID();
        UUID realCompanyId = UUID.randomUUID();
        UUID productId = UUID.randomUUID();

        Product product = product(productId, realCompanyId, ProductStatus.ACTIVE);
